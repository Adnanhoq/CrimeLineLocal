...
function CaseDetail() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      await uploadWitnessStatements(caseName, fileName, Buffer.from(fileContent), contentType);
      console.log('Uploaded:', fileName);
    }
}

    const uploadWitnessStatements = async (folderName: string, fileName: string, fileContent: Buffer, contentType: string) => {
  ...
  return (
    <div className="flex flex-col min-h-screen">
        <CrimeLineHeader title={caseName} showButton={true} navigatePath={"/"} buttonText={"Home"} />
        ...
        <form onSubmit={handleSubmit}>
            ...
            <button type="submit" className="rounded-md p-2 outline outline-1 hover:contrast-75">Upload</button>
        </form>
